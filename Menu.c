...
struct BlueWings_airline
{
	...
	int seat_num;
	char email[15];
	struct BlueWings_airline *following;
}
*begin, *stream;
struct BlueWings_airline *dummy;
...
void main()
{
	...
	do
	{
		...
		printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   welcome to Snehendu's airline system                   ");
		printf("\n\t\t ********************************************************************");
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct BlueWings_airline*)malloc(sizeof(struct BlueWings_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct BlueWings_airline *)malloc(sizeof(struct BlueWings_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
...
void savefile()
{
	FILE *fpointer = fopen("BlueWings_airline records", "w");
	if (!fpointer)
	{
	...
	printf("\n\n\t Details have been saved to a file (BlueWings_airline records)");
	fclose(fpointer);
}
